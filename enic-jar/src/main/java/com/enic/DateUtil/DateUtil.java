...
public class DateUtil {
	...
	public static Integer getMinuteValue(String timeValue){
		Integer minute  = null;
		if(StringUtils.isNotBlank(timeValue) && timeValue.contains(":")){
			String[] times = timeValue.split(":");
			if(times.length>0){
     ...
     * @return
     */
    public static Date getNextNWorkDay(Date start, int days){
        Date nextNWorkDay = start;
        int count = 0;
        int step = 1;
        if(days < 0) {
            step = -1;
        ...
        while(count != days){
            nextNWorkDay = addDays(nextNWorkDay,step);
            if(isWeekend(nextNWorkDay)) {
                continue;
            }
            count += step;
        }
    ...
    public static void main(String[] args) {
    	...
    	System.out.println(DateUtil.formatDate(DateUtil.getNextNWorkDay(now,15)));
